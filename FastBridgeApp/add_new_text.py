...
def import_(title, section_level, csv, language, local_def=False, local_lem=False):
    ...
    #rows are expected to be sanitzied to come in as :TITLE	LOCATION SECTION RUNNINGCOUNT TEXT (LOCAL_DEF TEXT_SPECIFIC_PRINCIPLE_PARTS), where SHORTDEF is the local definition and lemma is a local lemma (for dialectical differences) Those last two are optional
    ...
    for i in range(len(csv_reader)):
        ...
        try:
            ...
            return f"Error: the row: {row} seems to be missing something. Does it have all of: TITLE, LOCATION, SECTION, RUNNINGCOUNT, TEXT and what you specified of: LOCAL_DEF, TEXT_SPECIFIC_PRINCIPLE_PARTS"
