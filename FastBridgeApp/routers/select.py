...
def _process_vocabulary_data(triple, language):
    """
    Common logic for processing vocabulary data from text selections.
    Extracted to eliminate code duplication between simple_result and result routes.

    Returns: (titles, titles_no_dups, display_triple, count_in_text, local_def, local_lem, frequency_dict)
    """
    local_def = False
    local_lem = False
    titles = []
    ...
    # Remove duplicates and build frequency dictionary
    frequency_dict = {}
    dups = set()
    new_titles = []
    ...
    titles_no_dups = sorted(new_titles, key=lambda x: x[1])
    titles = sorted(titles, key=lambda x: x[1])

    return titles, titles_no_dups, display_triple, count_in_text, local_def, local_lem, frequency_dict
...
async def simple_result(request : Request, starts : str, ends : str, sourcetexts : str, language : str, running_list: str):
    ...
    titles, titles_no_dups, display_triple, count_in_text, local_def, local_lem, frequency_dict = _process_vocabulary_data(triple, language)
    ...
    # Filter words to match titles_no_dups without re-calling expensive mg_get_lang_data
    titles_no_dups_words = {title[0] for title in titles_no_dups}
    words_no_dups = []
    for word in words:
        title = word[0].TITLE
        if title in titles_no_dups_words:
            words_no_dups.append(word)
            titles_no_dups_words.discard(title)
...
async def result(request : Request, starts : str, ends : str, sourcetexts : str, in_exclude : str, othertexts : str, otherstarts : str, otherends : str, language : str, running_list: str):
    ...
    # Process source texts using common logic
    source = make_quads_or_trips(sourcetexts, starts, ends)
    titles, titles_no_dups, display_triple, count_in_text, local_def, local_lem, frequency_dict = _process_vocabulary_data(source, language)

    # Process "other" texts for filtering
    ...
    for text, start, end in other:
        locations_list, location_words = mg_get_locations(language, text, get_index=True)
        book = mg_get_text_as_Text(language, text, locations_list, location_words)
        other_titles = other_titles.union(set((book.get_words(start, end))))
        display_triple_other.append((book.name, start, end))
        del book

    # Extract just the words (first element) from other_titles
    other_titles = set([(new[0]) for new in other_titles])
    ...
    known = ", ".join(["{text}: {start} - {end}".format(text = text[0], start = text[1], end = text[2]) for text in display_triple_other])
    ...
    # Filter titles based on the include/exclude operation
...
def build_html_for_clusterize(words, POS_list, columnheaders, row_filters, style, context, frequency_dict, titles, global_filters, words_no_dups, titles_no_dups, language, count_in_text):
    ...
    for POS in POS_list:
        ...
        style+= new_style
        checks_parts.append(f'<div class="form-group"><div class="custom-control custom-checkbox"><input type="checkbox" name="filterChecks" class="custom-control-input" value="hide"  id="{POS}" onchange="hide_show_row(\'{POS}\');" checked><label class="custom-control-label" for="{POS}">{POS.replace("_", " ")}</label>')
        if filters:
            checks_parts.append(f'<span class="dropdown-submenu"> <button class="btn btn-no-padding" onclick="document.getElementById(\'{POS}extra\').classList.toggle(\'show\')">Refine</button><ul id= "{POS}extra" class="dropdown-menu" style = "border: 0px; color:inherit;background-color:gray;"">{filters}</ul> </span>')
        checks_parts.append('</div></div>')
    checks_parts.append('<label for="global filters"> Shortcut Row Filters</label><div id="global filters">')
    for global_f in global_filters:
        ...
        checks_parts.append(f'<div class="form-group"><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" value="hide"  id="{global_f}" onchange="global_filter(\'{global_f}\');" checked><label class="custom-control-label" for="{global_f}">{display_globalf}</label></div></div>')
    checks_parts.append('</div>')
    checks = ''.join(checks_parts)

    headers_parts = []
    other_headers_parts = []
    empty = [0]*len(columnheaders)
    empty2 = [True]*len(columnheaders)
    empty = [list(a) for a in zip(empty, empty2)]
    header_js_obj = dict(zip(columnheaders, empty)) #will be a javascript object for tracking filters
    ...
    for i in range(len(columnheaders)):
        headers_parts.append('<div class="form-group"> <div class="custom-control custom-checkbox">')
        if columnheaders[i] == "PRINCIPAL_PARTS" or columnheaders[i] == "SHORT_DEFINITION" or columnheaders[i] == "TEXT_SPECIFIC_DEFINITION":
            if columnheaders[i] == "SHORT_DEFINITION" and "TEXT_SPECIFIC_DEFINITION" in columnheaders:
                ...
                rules_added +=1
                headers_parts.append(f'<input type="checkbox" class="custom-control-input" value="show" id="{columnheaders[i]}" onchange="hide_show_column(\'{columnheaders[i]}\');">')
            else:
                headers_parts.append(f'<input type="checkbox" class="custom-control-input" value="hide" id="{columnheaders[i]}" onchange="hide_show_column(\'{columnheaders[i]}\');" checked>')
        ...
        other_headers_parts.append(f'<th id="{columnheaders[i]}_head" class="{columnheaders[i]}" onclick="sortTable(\'{columnheaders[i]}\',{i})" >{new_display.replace("_", " ").title()}</th>')
        headers_parts.append(f'<label class="custom-control-label" for="{columnheaders[i]}">{new_display.replace("_", " ").title()}</label></div></div>')

    headers = ''.join(headers_parts)
    other_headers = ''.join(other_headers_parts)
    ...
    for j in range(len(words)):
        ...
        for i in range(len(columnheaders)): #removing TITLE from the column headers makes things be o
            if(columnheaders[i] == "LOCAL_DEFINITION"):
                ...
                row_parts.append(f'<td class="{columnheaders[i]}">{words[j][0].TEXT_SPECIFIC_PRINCIPAL_PARTS}</td>')
                lst.append(words[j][0][-3])
            elif(columnheaders[i] =="LOGEION_LINK"):
                row_parts.append(f'<td class="{columnheaders[i]}"><a class="fa fa-external-link" style="font-size: 20px;" role="button" target="_blank" href="{words[j][0].LOGEION_LINK}"> </a></td>')
                lst.append(words[j][0].LOGEION_LINK)
            elif(columnheaders[i] =="FORCELLINI_LINK"):
                row_parts.append(f'<td class="{columnheaders[i]}"><a class="fa fa-external-link" style="font-size: 20px;" role="button" target="_blank" href="{words[j][0].FORCELLINI_LINK}"> </a></td>')
                lst.append(words[j][0].FORCELLINI_LINK)
            elif(columnheaders[i] == "Count_in_Selection"):
                row_parts.append(f'<td class="{columnheaders[i]}">{frequency_dict[words[j][0][0]]}</td>')
                lst.append(frequency_dict[words[j][0][0]])
            elif(columnheaders[i] == "Location"):
                row_parts.append(f'<td class="{columnheaders[i]}">{words[j][0].Appearance}</td>')
