...
def convert_to_csv(data_folder, data_type):
    ...
    for dirpath, dirnames, filenames in os.walk(f"../data/{data_folder}"):
        for spreadsheet_name in filenames:
            print("Current Spreadsheet: ", spreadsheet_name)

            if PurePosixPath(spreadsheet_name).suffix == ".csv":
                data = pd.DataFrame(pd.read_csv(f"../data/{data_folder}/{spreadsheet_name}"))
                ...
                data = pd.DataFrame(pd.read_excel(f"../data/{data_folder}/{spreadsheet_name}")) 
                cleaned_data_final = clean_data(data, spreadsheet_name, acceptable_headers, target_headers, necessary_headers, is_text_data)
...
def check_for_duplicates(spreadsheet_name, data):
    ...
    if column_header_duplicates.any():
        ...
        if not os.path.exists(f"../data/duplicate_data/"):
            os.mkdir(f"../data/duplicate_data/")
        if not os.path.exists(f"../data/duplicate_data/duplicate_columns/"):
            os.mkdir(f"../data/duplicate_data/duplicate_columns/")
        spreadsheet_csv_name = spreadsheet_name.strip(".xlsx")
    ...
    if row_duplicates.any():
        ...
        if not os.path.exists(f"../data/duplicate_data/"):
            os.mkdir(f"../data/duplicate_data/")
        if not os.path.exists(f"../data/duplicate_data/duplicate_rows/"):
            os.mkdir(f"../data/duplicate_data/duplicate_rows/")
        spreadsheet_csv_name = spreadsheet_name.strip(".xlsx")
